...
def render_report(
    *,
    ...
    alerts: Dict[str, Any],
    x402_spend_budget: Optional[Dict[str, Any]] = None,
) -> str:
    ...
    budget_badge = ""
    budget_note = ""
    if x402_spend_budget is not None:
        remaining = _esc(x402_spend_budget.get("remaining_usd", "unknown"))
        reserved = _esc(x402_spend_budget.get("reserved_usd", "unknown"))
        maximum = _esc(x402_spend_budget.get("max_usd_per_session", "unknown"))
        budget_badge = f'<span class="badge badge-readonly">x402 {remaining} left</span>'
        budget_note = (
            f"<p>x402 budget: {reserved} reserved of {maximum}. "
            "Scout reserves the full per-payment cap before each signature.</p>"
        )
...
def build_report(host: ReadOnlyHost) -> str:
    """Observe once, derive the full chain, and render one report."""
    data = host.build_report_data()
    if data["status"] != "ok":
        error = data.get("error", {})
        raise RuntimeError(
            f"Scout report stopped: {error.get('kind', 'observe')}: "
            f"{error.get('message', 'wallet observation failed')}"
        )
    return render_report(
        ...
        alerts=data["alerts"],
        x402_spend_budget=data.get("x402_spend_budget"),
    )
