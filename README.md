...
Before signing, Scout accepts only exact Base mainnet USDC requirements with an explicit recipient, a timeout no longer than 600 seconds, and an amount within the configured cap. Set `ZERION_X402_PAY_TO` to pin the one address it may pay; unset, any recipient the server names is accepted. It also refuses a paid retry that lacks the `PAYMENT-SIGNATURE` header.
